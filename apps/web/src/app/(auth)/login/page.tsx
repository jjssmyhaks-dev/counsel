...
export default function LoginPage() {
  ...
  const handleSso = async () => {
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json().catch(() => ({})); throw new Error(data.error || 'SSO initiation failed'); }
      ...
      <div className="hidden lg:flex w-[45%] relative overflow-hidden bg-gradient-to-br from-[#0c0a09] via-[#111c17] to-[#0c0a09] items-center justify-center">
        ...
        <div className="relative max-w-md px-12">
          ...
          <p className="mt-5 text-[15px] text-white/50 leading-relaxed">
            Document analysis, legal research, AI drafting, and knowledge management — all in one platform.
          </p>
          <div className="mt-10 space-y-3">
            {[
              "23 clause types auto-detected", "RAG-powered legal research with citations",
              "AI drafting in your firm's voice", "SOC 2 Type II · ISO 27001 · GDPR ready"
            ].map((c) => (
              <div key={c} className="flex items-center gap-2.5 text-[14px] text-white/60">
        ...
        <div className="w-full max-w-lg">
          ...
          <div className="mt-8 flex gap-1 p-1 rounded-xl bg-[#f0f0f0] w-fit">
            {[
              ...
              <button
                key={t.label}
                onClick={() => setShowSso(t.value)}
                className={`text-[13px] font-medium px-5 py-2 rounded-lg transition-all ${
                  showSso === t.value ? 'bg-white text-[#0c0a09] shadow-sm' : 'text-[#717d79] hover:text-[#0c0a09]'
                }`}
              >
                {t.label}
              </button>
          ...
          {error && (
            <div className="mt-4 bg-[#fdf0ee] border border-[#f0705b]/20 text-[#c2452e] px-4 py-3 rounded-xl text-[13px]">
              {error}
            </div>
          )}
          ...
          {!showSso ? (
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              <div>
                ...
                <input id="email" type="email" autoComplete="email" value={email}
                  onChange={(e) => setEmail(e.target.value)} placeholder="admin@sterling.law"
            ...
            <div className="mt-6 space-y-4">
              <div>
                <label htmlFor="sso-email" className="block text-[13px] font-medium text-[#0c0a09] mb-1.5">Work email</label>
                <input id="sso-email" type="email" autoComplete="email" value={email}
                  onChange={(e) => setEmail(e.target.value)} placeholder="you@yourfirm.com"
                  className="w-full px-4 py-3 rounded-xl border border-black/[0.08] bg-white text-[14px] text-[#0c0a09] placeholder:text-[#969e9b] focus:outline-none focus:ring-2 focus:ring-[#15b881]/30 focus:border-[#15b881]/40 transition-all" />
              </div>
              <button onClick={handleSso} disabled={loading}
                className="w-full bg-[#0c0a09] text-white py-3 rounded-xl text-[14px] font-medium hover:bg-[#0a8a5f] transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                {loading ? <><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg> Redirecting to your IdP...</> : 'Continue with SSO'}
              </button>
          ...
          <div className="mt-8 pt-8 border-t border-black/[0.04]">
            <div className="text-[13px] text-[#969e9b]">
              ...
              <Link href="/register" className="text-[#0a8a5f] font-medium hover:text-[#15b881] transition-colors">Create account</Link>
            </div>
            <div className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-2 text-[12px] text-[#969e9b]">
              {["Free 14-day trial", "No credit card", "SOC 2 Type II"].map((b) => (
