...
export default function KbPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <form onSubmit={handleSearch} className="max-w-2xl mx-auto">
        <div className="relative">
          ...
          <input
            ...
            className="block w-full pl-12 pr-24 py-3.5 text-base border border-slate-300 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 placeholder:text-slate-400"
          />
          <Button
            type="submit"
            loading={searching}
      ...
      {/* Error */}
      {error && (
        <div className="max-w-2xl mx-auto bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg text-sm flex items-center justify-between">
          <span>{error}</span>
          ...
          {result.answer ? (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-semibold text-slate-900 text-sm">Answer</h3>
            ...
            <div>
              ...
              <div className="space-y-2">
                {result.sources.map((source) => (
                  <div key={source.id} className="bg-slate-50 rounded-lg px-3 py-2.5 border border-slate-100">
                    <p className="text-xs font-medium text-slate-700">{source.title}</p>
      ...
      {!result && !searching && !error && (
        <div className="max-w-2xl mx-auto">
          <Card className="text-center py-16">
            ...
            <h3 className="text-lg font-semibold text-slate-900 mb-1">Ask your firm anything</h3>
      ...
      {history.length > 0 && (
        <div className="max-w-3xl mx-auto">
          <h3 className="text-sm font-semibold text-slate-700 mb-3">Recent Queries</h3>
          <div className="space-y-2">
            {history.slice(0, 5).map((item) => (
              <button
                key={item.id}
                onClick={() => { setQuestion(item.question); }}
                className="w-full text-left bg-white rounded-lg border border-slate-200 px-4 py-3 hover:bg-slate-50 transition-colors flex items-center justify-between group"
              >
                ...
                <div className="flex items-center gap-2 flex-shrink-0 ml-3">
                  ...
                  <svg className="w-4 h-4 text-slate-300 group-hover:text-blue-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
