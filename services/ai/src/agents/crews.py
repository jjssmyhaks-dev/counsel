...
# ─── CA Vertical Crew Orchestrators ──────────────────────────────────────────

def _serialize_ca_output(result) -> dict:
    """Serialize a CA crew result with audit metadata."""
    return {
        "raw_output": _serialize_output(result),
        "token_usage": dict(result.token_usage) if hasattr(result, "token_usage") and result.token_usage else {},
        "partner_review_required": True,
        "disclaimer": "This output was generated by Counsel AI. The signing CA bears professional responsibility. All filing-bound numbers require partner review before submission.",
    }


async def run_ca_bookkeeping_reconciliation(client_name: str = "Client", period: str = "Q1 2026",
    trial_balance_ref: str = "", bank_stmt_ref: str = "", step_callback=None) -> dict:
    """Crew 9: Bookkeeping Reconciliation pipeline."""
    from .definitions import get_agent
    from .tasks import CABookkeepingTasks
    sc = step_callback or (lambda x: None)
    matcher = get_agent("transaction_matcher")
    analyzer = get_agent("variance_analyzer")
    reporter = get_agent("reconciliation_reporter")
    tasks = CABookkeepingTasks(client_name, period, trial_balance_ref, bank_stmt_ref)
...
async def run_ca_gst(client_name: str = "Client", gstin: str = "", period: str = "", step_callback=None) -> dict:
    """Crew 10: GST & Indirect Tax pipeline."""
    from .definitions import get_agent
    from .tasks import CAGSTTasks
    sc = step_callback or (lambda x: None)
    itc = get_agent("input_tax_reconciler")
    validator = get_agent("gstr_validator")
    prep = get_agent("filing_prep_advisor")
    tasks = CAGSTTasks(client_name, gstin, period)
...
async def run_ca_audit(client_name: str = "Client", year: str = "2025-26", engagement_type: str = "Statutory Audit", step_callback=None) -> dict:
    """Crew 11: Audit & Assurance pipeline."""
    from .definitions import get_agent
    from .tasks import CAAuditTasks
    sc = step_callback or (lambda x: None)
    risk = get_agent("risk_assessment_engine")
    sample = get_agent("sampling_recommendation")
    compiler = get_agent("audit_report_compiler")
    tasks = CAAuditTasks(client_name, year, engagement_type)
...
async def run_ca_income_tax(client_name: str = "Client", pan: str = "", assessment_year: str = "2026-27", step_callback=None) -> dict:
    """Crew 12: Income Tax & TDS pipeline."""
    from .definitions import get_agent
    from .tasks import CAIncomeTaxTasks
    sc = step_callback or (lambda x: None)
    tds = get_agent("tds_reconciler")
    itr = get_agent("itr_data_aggregator")
    notice = get_agent("notice_response_drafter")
    tasks = CAIncomeTaxTasks(client_name, pan, assessment_year)
...
async def run_ca_roc(client_name: str = "Client", cin: str = "", step_callback=None) -> dict:
    """Crew 13: ROC/Corporate Compliance pipeline."""
    from .definitions import get_agent
    from .tasks import CAROCTasks
    sc = step_callback or (lambda x: None)
    tracker = get_agent("filing_deadline_tracker")
    compiler = get_agent("form_data_compiler")
    calendar = get_agent("compliance_calendar_manager")
    tasks = CAROCTasks(client_name, cin)
